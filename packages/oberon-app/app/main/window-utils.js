...
import { Curl } from 'oberon-libcurl';
import fs from 'fs';
...
export function createWindow() {
    ...
    submenu: [
      {
        ...
        click: () => {
          shell.openExternal('https://oberon.rest/support/');
        },
      },
      {
      ...
      {
        label: `Oberon ${MNEMONIC_SYM}Help`,
        accelerator: !isMac() ? 'F1' : null,
        click: () => {
          shell.openExternal('https://support.oberon.rest');
        },
      },
    ],
  };
  ...
  if (isDevelopment() || process.env.OBERON_FORCE_DEBUG) {
  ...
  dialog.showMessageBox(
    {
      ...
      cancelId: 0,
      title: 'Unresponsive',
      message: 'Oberon has become unresponsive. Do you want to reload?',
    },
    id => {
      if (id === 1) {
        mainWindow.destroy();
