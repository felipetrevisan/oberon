...
export function getAppEnvironment() {
  return process.env.OBERON_ENV || 'production';
}

export function getTempDir() {
  // NOTE: Using a fairly unique name here because "oberon" is a common word
  const { app } = electron.remote || electron;
  const dir = path.join(app.getPath('temp'), `oberon_${getAppVersion()}`);
  mkdirp.sync(dir);
  return dir;
}
...
export const GA_LOCATION = 'https://desktop.oberon.rest/';
export const CHANGELOG_BASE_URL = 'https://oberon.rest/changelog';
...
export const UPDATE_URL_MAC = 'https://updates.oberon.rest/builds/check/mac';
export const UPDATE_URL_WINDOWS = 'https://updates.oberon.rest/updates/win';

// API
export const API_BASE_URL = 'https://api.oberon.rest';
...
export const RESPONSE_CODE_DESCRIPTIONS = {
  ...
  [STATUS_CODE_PLUGIN_ERROR]:
    'An oberon plugin threw an error which prevented the request from sending',
