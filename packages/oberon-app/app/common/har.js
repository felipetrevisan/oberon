...
import { jarFromCookies } from 'oberon-cookies';
...
import { smartEncodeUrl } from 'oberon-url';
...
export async function exportHar(exportRequests: Array<ExportRequest>): Promise<Har> {
  ...
  return {
    log: {
      version: '1.2',
      creator: {
        name: 'Oberon REST Client',
        version: `oberon.desktop.app:v${getAppVersion()}`,
      },
      entries: entries,
    },
  };
...
function getRequestPostData(renderedRequest: RenderedRequest): HarPostData | void {
  ...
  if (renderedRequest.body.fileName) {
    ...
    // For every other type, Oberon uses the same body format as HAR
