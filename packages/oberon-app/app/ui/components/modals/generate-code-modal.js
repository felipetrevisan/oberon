...
class GenerateCodeModal extends PureComponent {
  constructor(props) {
    ...
    try {
      target = JSON.parse(window.localStorage.getItem('oberon::generateCode::target'));
    } catch (e) {}
    try {
      client = JSON.parse(window.localStorage.getItem('oberon::generateCode::client'));
    } catch (e) {}
  ...
  async _generateCode(request, target, client) {
    ...
    window.localStorage.setItem('oberon::generateCode::client', JSON.stringify(client));
    window.localStorage.setItem('oberon::generateCode::target', JSON.stringify(target));
  }
