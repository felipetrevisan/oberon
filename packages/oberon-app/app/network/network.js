...
import { cookiesFromJar, jarFromCookies } from 'oberon-cookies';
...
export async function _actuallySend(
  ...
  return new Promise(async resolve => {
    ...
    try {
      ...
      // If we calculated the body within Oberon (ie. not computed by Curl)
      if (requestBody !== null) {
      ...
      if (!hasUserAgentHeader(headers)) {
        setOpt(Curl.option.USERAGENT, `oberon/${getAppVersion()}`);
      }
